...
interface Props {
  ...
  openEditCashierModal: (cashier: Cashier) => void; // Функция открытия модального окна
...
const CashiersSection: React.FC<Props> = ({
  ...
  handleCreateCashier,
  openEditCashierModal, // Используется для открытия модального окна
  handleDeleteCashier,
}) => (
  <section className="space-y-10">
    ...
    <div className="bg-white/80 backdrop-blur-md border border-gray-200 shadow-xl rounded-2xl p-8 transition-all">
      <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center gap-2">
        <FiPlus className="text-blue-500" /> Новый кассир
      </h2>
    ...
    <div className="bg-white/80 backdrop-blur-md border border-gray-200 shadow-xl rounded-2xl p-8 transition-all">
      <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center gap-2">
        <FiUsers className="text-indigo-500" /> Список кассиров
      </h2>
      ...
      <div className="relative mb-6">
        <InputField
          ...
          placeholder="Поиск..."
        />
        <FiSearch className="absolute right-3 top-10 text-gray-400 pointer-events-none" />
      </div>
          ...
          {cashiers.map((cashier) => (
            ...
            >
              <div className="flex items-center gap-3">
                <FiUsers className="text-indigo-500 w-6 h-6 shrink-0" />
              ...
              <div className="flex gap-3 mt-4">
                <PrimaryButton
                  type="button"
                  onClick={() => openEditCashierModal(cashier)} // Открытие модального окна
